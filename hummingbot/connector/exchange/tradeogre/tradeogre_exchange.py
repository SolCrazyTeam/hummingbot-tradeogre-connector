...
class TradeogreExchange(ExchangePyBase):
    ...
    def authenticator(self):
        return TradeogreAuth(api_key=self.api_key, secret_key=self.secret_key, time_provider=self._time_synchronizer)
    ...
    def name(self) -> str:
        if self._domain == "com":
            ...
            return f"tradeogre_{self._domain}"
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        type_str = TradeogreExchange.tradeogre_order_type(order_type)
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(tradeogre_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/tradeogre-exchange/tradeogre-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Tradeogre the ID of the order being canceled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Tradeogre's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Tradeogre's get order endpoint does not return trade IDs.
        ...
        if long_interval_current_tick > long_interval_last_tick or (
            ...
            query_time = int(self._last_trades_poll_tradeogre_timestamp * 1e3)
            self._last_trades_poll_tradeogre_timestamp = self._time_synchronizer.time()
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(tradeogre_utils.is_exchange_information_valid, exchange_info["symbols"]):
