...
from hummingbot.connector.exchange.tradeogre.tradeogre_api_user_stream_data_source import (
    TradeogreAPIUserStreamDataSource,
)
...
class TradeogreExchange(ExchangePyBase):
    """
    TradeogreExchange connects with Tradeogre exchange and provides order book pricing, user account tracking and
    ...
    def __init__(
        ...
        tradeogre_secret_key: str,
        tradeogre_group_id: str,
        trading_pairs: Optional[List[str]] = None,
        trading_required: bool = True,
    ):
        """
        :param client_config_map: The config map of the client instance.
        :param tradeogre_api_key: The API key to connect to private Tradeogre APIs.
        :param tradeogre_secret_key: The API secret.
        :param trading_pairs: The market trading pairs which to track order book data.
        :param trading_required: Whether actual trading is needed.
        """
        self.tradeogre_api_key = tradeogre_api_key
        self.tradeogre_secret_key = tradeogre_secret_key
        self.tradeogre_group_id = tradeogre_group_id
    ...
    def authenticator(self):
        return TradeogreAuth(self.tradeogre_api_key, self.tradeogre_secret_key)
    ...
    async def get_all_pairs_prices(self) -> Dict[str, Any]:
        ...
        It returns the response of the exchange (expected to be used by the Tradeogre RateSource for the RateOracle)
    ...
    async def _api_request_url(self, path_url: str, is_auth_required: bool = False) -> str:
        ...
        if is_auth_required:
            url = url.format(group_id=self.tradeogre_group_id)
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        api_path = CONSTANTS.ORDER_PATH_URL
        if trade_type is TradeType.BUY:
            api_path = api_path + "/buy"
        else:
            api_path = api_path + "/sell"
        ...
        if exchange_order["success"]:
            return (
                str(exchange_order["uuid"]),
                utils.get_ms_timestamp(),
            )
        else:
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        cancel_result = await self._api_post(
            path_url=CONSTANTS.ORDER_PATH_URL + "/cancel",
            data=data,
            is_auth_required=True,
        )
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://Tradeogre.github.io/Tradeogre-pro-api/#channel-order-and-balance
                ...
                # We need to use the offline balance estimation for Tradeogre
    ...
    async def _update_balances(self):
        ...
        if response.get("success"):
            for asset_name, value in response["balances"].items():
                self._account_available_balances[asset_name] = Decimal(value)
                self._account_balances[asset_name] = Decimal(value)
            ...
            self.logger().error(f"There was an error during the balance request to Tradeogre ({response})")
            raise IOError(f"Error requesting balances from Tradeogre ({response})")
    ...
    async def _all_trade_updates_for_order(self, order: InFlightOrder) -> List[TradeUpdate]:
        # Tradeogre does not have an endpoint to retrieve trades for a particular order
    ...
    async def _all_trade_updates_for_orders(
        ...
        trade_updates = []
        for order_fill_data in result:
            max_sequence_number = max(max_sequence_number, order_fill_data["sn"])
            if order_fill_data["uuid"] in orders_to_process:
                order_id = order_fill_data["uuid"]
                try:
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        try:
            updated_order_data = await self._api_get(
                path_url=CONSTANTS.ORDER_STATUS_PATH_URL + "/" + exchange_order_id, params=params, is_auth_required=True
            )

            if updated_order_data.get("success"):
